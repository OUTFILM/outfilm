...
function showSlideshow() {
	...
	$query = "SELECT * FROM spotlight ORDER BY time DESC LIMIT 5";
...
<div id="site_container">
    ...
    foreach($tags as $tag) { ?><div class="tag" onclick="featureTag('<?=$tag?>')"><div class="tag_hole"></div><p><?=$tag?></p></div><? }
    ?>
    </div>
    <!-- tags 
    <div id="tag_cont" style="margin-left: auto;margin-right:auto; margin-top: 20px; border-radius: 8px">
...
function featureTag(tag) { // Occurs when tag is clicked

	page = 1;
	var runOnce = 0;	
	myTag = tag;	
	
	if (isTagActivated == 0) { 	// Fade out slideshow and filmography sections only
...
function Disply_Latest(Link, Tag, Headline, Content, Datetime, Catalog_Id, column, delayTime) {	
	column = column % 3;
    /*<div class="Tag_display_latest"><p>' + Tag + '</p></div>*/
	$("#column_"+column).append(
		$('<div class="latest_box"><div class="latest_picbox">' +
		  ...
		  '</a></div><div class="Headline_display_latest">' +
        	...
        	$.each(output, function(key,value) {       			
				 if (value.Title != "null") {
					column = Disply_Latest(value.Link, value.Tag, value.Headline, 
								value.Content, value.Datetime, value.Catalog_Id, column, 300); 
				 }        			
        	}); 
        	$(".button-fill").show();  
        	if (is_click == 1) { // Occurs only after page is loaded (Load more is clicked)
...
$(document).ready(function(){
    var gettag
    if (gettag = '<?if(isset($_GET["key"])) echo $_GET["key"];?>') {
