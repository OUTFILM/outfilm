...
!function($) {
  ...
  self.prototype.setHTML = function(text, recurring) {
    ...
    text = this.clean(text, true, false);
  ...
  self.prototype.processPaste = function() {
    ...
    if (content.match(/(class=\"?Mso|style=\"[^\"]*\bmso\-|w:WordDocument)/gi)) {
      ...
      text = this.clean(content, false, true);
      text = this.removeEmptyTags(text);
      if (self.copiedText) {
  ...
  self.prototype.wordClean = function(code) {
    ...
    return code = code.replace(/<lilevel([^1])([^>]*)>/gi, '<li data-indent="true"$2>'), code = code.replace(/<lilevel1([^>]*)>/gi, "<li$1>"), code = this.clean(code), code = code.replace(/<a>(.[^<]+)<\/a>/gi, "$1");
  };
  /**
   * @param {number} count
   * @return {?}
    ...
    if (this.$link_wrapper.find("input.f-lu").removeClass("fr-error"), this.imageMode) {
      ...
      if (this.isLink) {
        ...
        if (this.options.linkText || "" === this.text()) {
          this.insertHTML('<span class="f-marker" data-fr-verified="true" data-id="0" data-type="true"></span>' + (val || this.clean(text)) + '<span class="f-marker" data-fr-verified="true" data-id="0" data-type="false"></span>');
          this.restoreSelectionByMarkers();
        }
  ...
  $.Editable.prototype.enterInMainElement = function(el) {
    ...
    if ($(element).parent().get(0) == el) {
      ...
      if ($(element).parents(this.$element).length) {
        ...
        for (;"BREAK" == el.tagName || 0 === $(el).text().length && el.parentNode != this.$element.get(0);) {
          el = el.parentNode;
        }
