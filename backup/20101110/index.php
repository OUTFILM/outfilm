     ...
     <script>
		...
		var win_size = $(window).width();
	 	...
	 	function Disply_Top1(row, picsrc, headline, content, catid, top1) {
	 		//console.log(row);
 			if(row==1)
			$('#ChatRoom').append('<div class="FlexChat_img"><img width="100%" src="'+picsrc+'" align="middle"/></div><div class="FlexChat_headline"><h4><a href="/Article/article.php?catlogid='+catid+'">'+headline+'</a></h4><p>'+content+'...</p></div>');
		     	...
		     	success: function(result) {
		        	...
		        	$.each(output1, function(key,value) {       			
						 if (value.Title != "null") {					 
							...
							top1 = Disply_Top1(value.Row, value.Link, value.Headline, text, value.Catalog_Id, top1);
							//console.log(top1);	
						 }        			
		        	});      	  
		       },            
